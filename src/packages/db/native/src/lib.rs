// packages/db/native/src/lib.rs
//
// Native part of the `db` package: SQLite via rusqlite + PostgreSQL over TCP
// (wire protocol v3).
//
// Built as a cdylib (liphia_package_db) and shipped prebuilt in
// packages/db/lib/. The VM loads it through liphia_virtual_machine::external,
// driven by packages/db/index.lph, when the package is installed.
//
...
//   md-5 = "0.10"
//
// "bundled" compiles SQLite from C source, so no system install is needed.
//
...
impl SqliteConn {
    ...
    fn open_memory() -> Result<Self, String> {
        let conn = SqliteConnection::open_in_memory().map_err(|e| e.to_string())?;
...
fn sqlite_value_to_liphia(val: ValueRef) -> Value {
    match val {
        ...
        ValueRef::Text(s) => Value::Str(Rc::new(std::str::from_utf8(s).unwrap_or("").to_string())),
        ValueRef::Blob(b) => Value::Str(Rc::new(format!("<blob {} bytes>", b.len()))),
    }
}
...
fn sqlite_query_flat(conn: &SqliteConn, sql: &str) -> Result<Value, String> {
    ...
    while let Some(row) = rows.next().map_err(|e| e.to_string())? {
        for i in 0..col_count {
            ...
            flat.push(sqlite_value_to_liphia(
                row.get_ref(i).unwrap_or(ValueRef::Null),
            ));
        }
    }
...
fn sqlite_query_rows(conn: &SqliteConn, sql: &str) -> Result<Value, String> {
    ...
    while let Some(row) = rows.next().map_err(|e| e.to_string())? {
        ...
        for i in 0..col_count {
            ...
            flat_row.push(sqlite_value_to_liphia(
                row.get_ref(i).unwrap_or(ValueRef::Null),
            ));
        }
...
fn native_db_open(args: Vec<Value>) -> VmResult<Value> {
    ...
    let conn = SqliteConn::open(&path).map_err(|e| VmError::new(format!("db_open: {}", e)))?;
...
fn native_db_open_memory(_args: Vec<Value>) -> VmResult<Value> {
    let conn =
        SqliteConn::open_memory().map_err(|e| VmError::new(format!("db_open_memory: {}", e)))?;
    let handle = alloc_handle();
    SQLITE_CONNS.with(|c| c.borrow_mut().insert(handle, conn));
    Ok(Value::Int(handle))
}
...
fn native_db_exec(args: Vec<Value>) -> VmResult<Value> {
    if args.len() != 2 {
        return Err(VmError::new(
            "db_exec(handle: int, sql: str) — expected 2 arguments",
        ));
    }
...
fn native_db_query(args: Vec<Value>) -> VmResult<Value> {
    if args.len() != 2 {
        return Err(VmError::new(
            "db_query(handle: int, sql: str) — expected 2 arguments",
        ));
    }
...
fn native_db_query_rows(args: Vec<Value>) -> VmResult<Value> {
    if args.len() != 2 {
        return Err(VmError::new(
            "db_query_rows(handle: int, sql: str) — expected 2 arguments",
        ));
    }
...
fn native_db_columns(args: Vec<Value>) -> VmResult<Value> {
    if args.len() != 2 {
        return Err(VmError::new(
            "db_columns(handle: int, table: str) — expected 2 arguments",
        ));
    }
...
fn pg_connect_inner(
    ...
    loop {
        ...
        match tag {
            b'R' => {
                if body.len() < 4 {
                    return Err("bad AuthenticationRequest".to_string());
                }
                ...
                match auth_type {
                    ...
                    5 => {
                        // AuthenticationMD5Password — next 4 bytes after the
                        // auth-type code are the salt.
                        if body.len() < 8 {
                            return Err(
                                "pg_connect: malformed AuthenticationMD5Password".to_string()
                            );
                        }
                        let salt: [u8; 4] = [body[4], body[5], body[6], body[7]];
                        let hashed = pg_md5_password(user, password, &salt);
                        pg_send_password(&mut stream, &hashed)?;
                    }
                    _ => {
                        return Err(format!(
        ...
        match tag {
            ...
            b'T' => {
                ...
                for _ in 0..field_count {
                    ...
                    let name = std::str::from_utf8(&body[start..pos])
                        .unwrap_or("?")
                        .to_string();
                    col_names.push(name);
                    pos += 1; // skip null
...
// PostgreSQL's MD5 auth challenge: md5(md5(password + username) + salt),
// prefixed with "md5" so the server recognizes it as a hashed response
// rather than a raw cleartext password.
...
fn native_pg_connect(args: Vec<Value>) -> VmResult<Value> {
    ...
    let stream = pg_connect_inner(&host, port, &user, &pass, &db).map_err(|e| VmError::new(e))?;
    let handle = alloc_handle();
    PG_CONNS.with(|c| {
        c.borrow_mut().insert(
            handle,
...
fn native_pg_exec(args: Vec<Value>) -> VmResult<Value> {
    if args.len() != 2 {
        return Err(VmError::new(
            "pg_exec(handle: int, sql: str) — expected 2 arguments",
        ));
    }
...
fn native_pg_query(args: Vec<Value>) -> VmResult<Value> {
    if args.len() != 2 {
        return Err(VmError::new(
            "pg_query(handle: int, sql: str) — expected 2 arguments",
        ));
    }
...
fn native_pg_query_rows(args: Vec<Value>) -> VmResult<Value> {
    if args.len() != 2 {
        return Err(VmError::new(
            "pg_query_rows(handle: int, sql: str) — expected 2 arguments",
        ));
    }
